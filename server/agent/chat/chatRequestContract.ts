import { z } from 'zod';
import { AIConfigSchema, type AIConfig } from '../../../shared/contracts/ai';
import { AgentClientContextSchema, type AgentClientContext } from '../../../shared/contracts/capability';
...
const CommonSchema={
  stateDelta:AgentClientContextSchema.optional(),
...
export function parseStrictAgentChatRequest(body:unknown):StrictAgentChatRequest{
 ...
 if('toolResponse' in value&&value.toolResponse)return{kind:'toolResponse',stateDelta:value.stateDelta||{},aiConfig:value.aiConfig,newMessage:value.toolResponse};
 return{kind:'message',stateDelta:value.stateDelta||{},aiConfig:value.aiConfig,newMessage:{role:'user',parts:[{text:value.message}]},...(value.attachments?{attachments:value.attachments}: {})};
}
