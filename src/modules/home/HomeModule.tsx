  ...
  return (
    <div className="mx-auto max-w-6xl space-y-4">
      <section className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-xs font-medium text-neutral-500">Không gian làm việc cá nhân</p>
          <h1 className="mt-0.5 text-2xl font-semibold tracking-tight text-neutral-950">Trang chủ</h1>
      ...
      {widgets.length > 0 && (
        <section aria-labelledby="home-attention-title" className="space-y-2">
          <div>
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            {widgets.map(widget => {
              ...
              const fullWidth = widgets.length === 1 || widget.width === 'full';
              return (
                <div key={widget.id} className={fullWidth ? 'md:col-span-2' : ''}>
                  <WidgetComponent />
                </div>
              );
            })}
      ...
      <section aria-labelledby="home-actions-title" className="space-y-2">
          ...
          {quickActions.map(action => {
            ...
            return (
              <button
                ...
                className="group flex min-h-20 items-start gap-3 rounded-xl border border-neutral-200 bg-white p-3 text-left shadow-2xs transition hover:border-neutral-300 hover:shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-neutral-900 focus-visible:ring-offset-2"
              >
                <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-neutral-100 text-neutral-800">
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="mt-0.5 block text-xs leading-relaxed text-neutral-500">{action.description}</span>
                </span>
              </button>
            );
          })}
      ...
      <section aria-labelledby="home-files-title" className="space-y-2">
        <div>
