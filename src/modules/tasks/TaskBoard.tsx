...
export type TaskItem = TaskFormValue & { id: string; createdAt?: string | null; updatedAt?: string | null };
...
const formatDueDate = (value: string) => value ? new Intl.DateTimeFormat('vi-VN').format(new Date(`${value}T00:00:00`)) : '';
...
export function TaskBoard({ tasks, canWrite, busyId, onOpenTask, onMoveTask, onQuickCreate }: TaskBoardProps) {
  ...
  const submitQuickCreate = async (status: TaskStatus) => {
    ...
    if (!title) return;
    await onQuickCreate({ title, description: '', status, priority: 'medium', category: 'Công việc', dueDate: '' });
    setQuickTitle('');
    setQuickColumn(null);
  };
            ...
            <section key={column.status} className="min-w-0 rounded-2xl border border-neutral-200 bg-neutral-50/80 p-3" aria-labelledby={`task-column-${column.status}`}>
              <div className="mb-3 flex items-center justify-between gap-2 px-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="rounded-full bg-white px-2 py-0.5 text-xs font-medium text-neutral-500 ring-1 ring-neutral-200">{columnTasks.length}</span>
                </div>
              ...
              <div className="space-y-2">
                {columnTasks.length === 0 ? (
                  ...
                  const overdue = Boolean(task.dueDate && task.status !== 'completed' && task.dueDate < todayKey());
                  return (
                    <article key={task.id} className={`group relative rounded-xl border bg-white p-3 shadow-sm transition-shadow hover:shadow-md ${overdue ? 'border-rose-200' : 'border-neutral-200'}`}>
                      <button type="button" onClick={() => onOpenTask(task)} className="block w-full pr-8 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-neutral-900 rounded-lg" aria-label={`Mở chi tiết ${task.title}`}>
                        ...
                        <div className="mt-2 flex flex-wrap items-center gap-2 text-xs">
                          ...
                          {task.dueDate && <span className={`inline-flex items-center gap-1 ${overdue ? 'font-medium text-rose-700' : 'text-neutral-500'}`}><Calendar className="h-3.5 w-3.5" />{overdue ? 'Quá hạn · ' : ''}{formatDueDate(task.dueDate)}</span>}
