...
const userDefaults = permissions.match(/USER_DEFAULT_PERMISSIONS\s*=\s*\[([\s\S]*?)\] as const/)?.[1] || '';
check('normal user can read personal settings', userDefaults.includes("'settings.read'"));
